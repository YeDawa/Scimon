    ...
    pub fn rules(&self, script_content: &str) -> (Vec<String>, bool) {
        let rules = vec![
            ...
            Rules {
                ...
                // Correção: Usando r#"..."# para permitir o uso de ['"] com segurança
                pattern: Regex::new(r#"(?i)(require\(['"]winreg['"]\)|reg\s+add|reg\.exe)"#).unwrap(),
            },
            Rules {
            ...
            Rules {
                ...
                // Correção: Usando r#"..."# aqui também devido aos requires
                pattern: Regex::new(r#"(?i)(require\(['"](http|https|net|dgram|axios|node-fetch)['"]\)|fetch\(|XMLHttpRequest|curl\s+|wget\s+|nc\s+)"#).unwrap(),
            },
